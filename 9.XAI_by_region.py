...
importnces_by_region = defaultdict(float)
for i in range(100):
    # Train your RFC model
    X_train, X_test, y_train, y_test = train_test_split(features, target, test_size=0.2, random_state=5 * i)
    ...
    for region, indexes in FACE_INDEXES.items():
        for index in indexes:
            # importnces_by_region[region] += importances_by_index[f"{region}_{index}_x"] + importances_by_index[
            #     f"{region}_{index}_y"]
            importnces_by_region[region] += importances_by_index[f"{region}_{index}"]

importnces_by_region = dict(sorted(importnces_by_region.items(), key=lambda x: x[1], reverse=True))
# for region, importance in importnces_by_region.items():
#     print(f"{region}: {importance}")
# export to csv
importances_df = pd.DataFrame(importnces_by_region.items(), columns=["Region", "Importance"])
importances_df.to_csv("importances_by_region.csv", index=False)
print([i for i in importnces_by_region.keys()])
print("Done")
